...
//       this.apiKey = 'AIzaSyAZkjm5rz6_VUbf-PX0JGoWsfFt4hwZh78';
//       this.city = city;
//       this.state = state;
//     }
...
const api_key = 'AIzaSyAZkjm5rz6_VUbf-PX0JGoWsfFt4hwZh78';
const search_button = document.getElementById('search-button');
const search_input = document.getElementById('book-search');
...
async function asyncCall(query) {
  try {
    ...
    let data = await axios.get(
      `https://www.googleapis.com/books/v1/volumes?q=${query}s&key=${api_key}`
    );
...
function display(data) {
  ...
  for (let i = 0; i < dataArray.length; i++) {
    if (dataArray[i].volumeInfo.authors == undefined) {
      dataArray[i].volumeInfo.authors = ['#Missing Entry#'];
    }
    if (dataArray[i].volumeInfo.publisher == undefined) {
      dataArray[i].volumeInfo.publisher = '#Missing Entry#';
    }
    ...
    const element = `
      <div class="uk-card uk-card-default uk-card-hover uk-grid-collapse uk-child-width-1-2@s" uk-grid>
        <div class="uk-card-media-left uk-cover-container">
          <img src="${
            dataArray[i].volumeInfo.imageLinks.thumbnail
          }" alt="The image to ${dataArray[i].volumeInfo.title} by ${
      dataArray[i].volumeInfo.authors[0]
    }" uk-cover>
        ...
        <div>
          <div class="uk-card-body">
            ...
            <a href="${
              dataArray[i].volumeInfo.previewLink
            }" class="uk-button uk-button-primary">Further Info</a>
          </div>
        </div>
      </div>
    `;
